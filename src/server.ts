...
const authCodes   = new Map<string, { redirect_uri: string; state?: string }>();
...
app.post("/register", (_req, res) => {
  const clientId = "advbox-claude-" + crypto.randomBytes(8).toString("hex");
  res.status(201).json({
    client_id: clientId,
...
app.post("/token", (req, res) => {
  ...
  if (validTokens.size > 1000) validTokens.clear();
