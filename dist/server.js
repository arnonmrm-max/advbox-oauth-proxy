...
app.post("/register", (_req, res) => {
    res.status(201).json({
        client_id: "advbox-" + crypto_1.default.randomBytes(8).toString("hex"),
...
async function callAdvbox(tool, args) {
    const r = await (0, node_fetch_1.default)(`${ADVBOX_URL}/execute`, {
        method: "POST",
        headers: {
...
async function callAdvboxRest(method, path, body) {
    const r = await (0, node_fetch_1.default)(`${ADVBOX_REST_URL}/${path}`, {
        method,
        headers: {
...
async function sendWhatsApp(phone, message, send_date) {
    ...
    if (send_date)
        params.set("send_date", send_date);
    const r = await (0, node_fetch_1.default)(CHATGURU_URL, {
        method: "POST",
    ...
    try {
        ...
        if (body.method === "tools/call") {
            ...
            if (name === "add_andamento") {
                const { lawsuit_id, descricao } = args;
                ...
                const lawsuit = await callAdvbox("get_lawsuit", { lawsuit_id });
                const notesAtual = lawsuit.notes || "";
                ...
                const notesNovo = notesAtual
                    ...
                    : novaLinha;
                // 4. Salva no processo
                const resultado = await callAdvbox("update_lawsuit", { lawsuit_id, notes: notesNovo });
                return res.json({
                    jsonrpc: "2.0", id,
            ...
            if (name === "create_movement") {
                const { lawsuit_id, description, date } = args;
            ...
            if (name === "send_whatsapp") {
                const { phone, message, send_date } = args;
                if (!phone || !message) {
...
app.post("/webhook/chatguru", async (req, res) => {
    try {
        ...
        const phone = (body.celular || body.phone || body.chat_number || "").replace(/\D/g, "");
        const nome = body.nome || body.name || "Cliente";
        const mensagem = body.texto_mensagem || body.message || body.text || "";
        const linkChat = body.link_chat || "";
        const responsavel = body.responsavel_nome || "";
        const chatCreated = body.chat_created || new Date().toISOString();
        // Tags: aceita array ["CLIENTE ATIVO"] ou string "CLIENTE ATIVO, OUTRO"
        const tagsRaw = body.tags || "";
        const tags = Array.isArray(tagsRaw)
            ? tagsRaw.map((t) => t.toUpperCase().trim())
            : String(tagsRaw).toUpperCase().split(/[,;]/).map((t) => t.trim());
        ...
        for (const p of phoneVariants) {
            const result = await callAdvboxRest("GET", `customers?phone=${p}&limit=5`);
        ...
        const lawsuitsResult = await callAdvboxRest("GET", `lawsuits?customer_id=${customer.id}&limit=100`);
        const lawsuits = lawsuitsResult?.data || [];
        ...
        const sorted = lawsuits.sort((a, b) => {
        ...
        if (mensagem)
            descricao += `: ${mensagem.substring(0, 300)}`;
        if (responsavel)
            descricao += ` [Atendente: ${responsavel}]`;
        if (linkChat)
            descricao += ` | Chat: ${linkChat}`;
        // Garante mínimo de 10 caracteres
        if (descricao.length < 10)
            descricao = `Contato via WhatsApp em ${hoje} — Nova conversa iniciada`;
...
app.post("/webhook/chatguru-summary", async (req, res) => {
    try {
        ...
        if (!chatId)
            return res.status(400).json({ error: "Link do chat não contém o hash/id do chat. Envie o link_chat no payload." });
        ...
        const history = await (0, scraper_1.scrapeChatHistory)(chatId);
        // 2. Passa para o Gemini resumir
        console.log(`[IA] Gerando resumo com Gemini...`);
        const summary = await (0, ai_1.generateSummary)(history);
